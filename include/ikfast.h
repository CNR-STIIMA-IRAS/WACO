...
#define  ACCDEC __host__ __device__ 
#include <cuda_vector.h>
using namespace cuda_std;
...
{
public:
  ACCDEC IkSingleDOFSolutionBase() : fmul(0), foffset(0), freeind(-1), maxsolutions(1)
  {
...
//   ACCDEC virtual void GetSolution(T* solution, const T* freevalues) const = 0;
// 
//   /// \brief vector version of \ref GetSolution
//   ACCDEC virtual void GetSolution(vector<T>& solution, const vector<T>& freevalues) const
//   {
...
//   ACCDEC virtual const vector<int>& GetFree() const = 0;
// 
//   /// \brief the dof of the solution
//   ACCDEC virtual const int GetDOF() const = 0;
// };
...
// {
// public:
//   ACCDEC virtual ~IkSolutionListBase()
//   {
//   }
// 
...
//   /// freely set.
//   ACCDEC virtual size_t AddSolution(const vector<IkSingleDOFSolutionBase<T> >& vinfos, const vector<int>& vfree) = 0;
// 
//   /// \brief returns the solution pointer
//   ACCDEC virtual const IkSolutionBase<T>& GetSolution(size_t index) const = 0;
// 
//   /// \brief returns the number of solutions stored
//   ACCDEC virtual size_t GetNumSolutions() const = 0;
// 
//   /// \brief clears all current solutions, note that any memory addresses returned from \ref GetSolution will be
//   /// invalidated.
//   ACCDEC virtual void Clear() = 0;
// };
/// \brief Default implementation of \ref IkSolutionBase
template <typename T>
class IkSolutionBase //: public IkSolutionBase<T>
{
public:
  ACCDEC  IkSolutionBase(){}
  ACCDEC  IkSolutionBase(const vector<IkSingleDOFSolutionBase<T> >& vinfos, const vector<int>& vfree)
  {
    _vbasesol = vinfos;
    _vfree = vfree;
  }

  ACCDEC void GetSolution(T* solution, const T* freevalues) const
  {
    //assert( _vbasesol.size() );
    //assert( _vbasesol.size() == 8 );
    
    for (std::size_t i = 0; i < _vbasesol.size(); ++i)
    {
      ; //      printf("%d# freeind: %d ", i, 0 );
...
// 	printf("%d#", i);
  ...
  ACCDEC  void GetSolution(vector<T>& solution, const vector<T>& freevalues) const
  {
    assert(GetDOF());
    solution.resize(GetDOF());
    GetSolution(&solution.at(0), freevalues.size() > 0 ? &freevalues.at(0) : NULL);
  ...
  ACCDEC  const vector<int>& GetFree() const
  {
    return _vfree;
  }
  ACCDEC  const int GetDOF() const
  {
    return static_cast<int>(_vbasesol.size());
  }

  ACCDEC  void Validate() const
  {
    for (size_t i = 0; i < _vbasesol.size(); ++i)
    {
      if (_vbasesol[i].maxsolutions == (unsigned char)-1)
      {
        RUNTIME_ERROR("max solutions for joint not initialized");
      }
      if (_vbasesol[i].maxsolutions > 0)
      {
        if (_vbasesol[i].indices[0] >= _vbasesol[i].maxsolutions)
        {
          RUNTIME_ERROR("index >= max solutions for joint");
        }
        if (_vbasesol[i].indices[1] != (unsigned char)-1 && _vbasesol[i].indices[1] >= _vbasesol[i].maxsolutions)
        {
          RUNTIME_ERROR("2nd index >= max solutions for joint");
        }
      }
    }
  }

  ACCDEC  void GetSolutionIndices(vector<unsigned int>& v) const
  {
    v.resize(0);
    v.push_back(0);
  ...
  vector<IkSingleDOFSolutionBase<T> > _vbasesol;  ///< solution and their offsets if joints are mimiced
  vector<int> _vfree;
};

/// \brief Default implementation of \ref IkSolutionListBase
template <typename T>
class IkSolutionListBase //: public IkSolutionListBase<T>
{
public:
  ACCDEC IkSolutionListBase()
  {
  }
  ACCDEC  size_t AddSolution(const vector<IkSingleDOFSolutionBase<T> >& vinfos, const vector<int>& vfree)
  {
    size_t index = _listsolutions.size();
    _listsolutions.push_back(IkSolutionBase<T>(vinfos, vfree));
    return index;
  }

  ACCDEC  const IkSolutionBase<T>& GetSolution(size_t index) const
  {
    if (index >= _listsolutions.size())
    {
      RUNTIME_ERROR("GetSolution index is invalid");
    }
//     typename vector<IkSolution<T> >::const_iterator it = _listsolutions.begin();
//     std::advance(it, index);
    return _listsolutions[index];
  }

  ACCDEC  size_t GetNumSolutions() const
  {
...
protected:
  vector<IkSolutionBase<T>> _listsolutions;
};
...
// {
// public:
//   ACCDEC explicit IkSolution()
//   {
//   }
//   ACCDEC IkSolution(const vector<IkSingleDOFSolutionBase<T> >& vinfos, const vector<int>& vfree)
//   {
//     _vbasesol = vinfos;
//     _vfree = vfree;
//   }
// 
//   ACCDEC virtual void GetSolution(T* solution, const T* freevalues) const
//   {
//     printf("_vbasesol.size() %d", _vbasesol.size());
//     //assert( _vbasesol.size() );
//     //assert( _vbasesol.size() == 8 );
//     /*
//     for (std::size_t i = 0; i < _vbasesol.size(); ++i)
...
//   }
// 
//   ACCDEC virtual const vector<int>& GetFree() const
//   {
//     return _vfree;
//   }
//   ACCDEC virtual const int GetDOF() const
//   {
//     return static_cast<int>(_vbasesol.size());
//   }
// 
//   ACCDEC virtual void Validate() const
//   {
//     for (size_t i = 0; i < _vbasesol.size(); ++i)
//     {
//       if (_vbasesol[i].maxsolutions == (unsigned char)-1)
...
//   }
// 
//   ACCDEC virtual void GetSolutionIndices(vector<unsigned int>& v) const
//   {
//     v.resize(0);
//     v.push_back(0);
//     for (int i = (int)_vbasesol.size() - 1; i >= 0; --i)
...
// {
// public:
//   ACCDEC void IkSolutionList()
//   {
// 	// nothing to do so far
//   }
//   ACCDEC virtual size_t AddSolution(const vector<IkSingleDOFSolutionBase<T> >& vinfos, const vector<int>& vfree)
//   {
//     size_t index = _listsolutions.size();
...
//   }
// 
//   ACCDEC virtual const IkSolutionBase<T>& GetSolution(size_t index) const
//   {
//     if (index >= _listsolutions.size())
//     {
//       RUNTIME_ERROR("GetSolution index is invalid");
...
//   }
// 
//   ACCDEC virtual size_t GetNumSolutions() const
//   {
//     return _listsolutions.size();
//   }
// 
//   ACCDEC virtual void Clear()
//   {
//     _listsolutions.clear();
//   }
// 
// protected:
//   vector<IkSolution<T> > _listsolutions;
// };
}
...
#ifdef _MSC_VER
#define IKFAST_API ACCDEC extern "C" __declspec(dllexport)
#else
#define IKFAST_API ACCDEC extern "C"
#endif
#else
#define IKFAST_API ACCDEC
#endif
